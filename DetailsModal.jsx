...
const DetailsModal = ({ title, data, columns, loading, onClose }) => {
  ...
  const periodSnapshot = useMemo(() => {
    ...
    const uniqueApps = new Set(filteredRows.map(r => r.Application).filter(Boolean)).size;
    const values = filteredRows.map(r => parseFloat(r.Value)).filter(v => !isNaN(v));
    const avg = values.length ? (values.reduce((a, b) => a + b, 0) / values.length).toFixed(1) : '-';
    const uniqueNARs = new Set(filteredRows.map(r => r.NAR_ID).filter(Boolean)).size;
  ...
  // 7. Repeat Offenders (apps appearing more than once)
                ...
                {top3Apps.length > 0 && (
                  <div className="insight-card">
                    ...
                    <div className="top-apps-list">
                      {top3Apps.map((app, i) => (
                        <div className="top-app-row" key={i} style={{ borderLeft: `3px solid ${medalColors[i]}` }}>
                ...
                {lowestPerformer && (
                  <div className="insight-card">
                    ...
                    <div className="insight-highlight-box red">
                      ...
                      <span className="insight-sub-text">Value: <strong>{parseFloat(lowestPerformer.Value).toFixed(1)}</strong></span>
                  ...
                  <div className="insight-card">
                    ...
                    <div className="distribution-list">
                      <div className="dist-row">
                        <span className="dist-label low">🟢 High (&gt;50)</span>
                      ...
                      <div className="dist-row">
                        <span className="dist-label mid">🟡 Mid (21–50)</span>
                        <span className="dist-bar-wrap">
                          <span
                      ...
                      <div className="dist-row">
                        <span className="dist-label high">🔴 Low (≤20)</span>
                        <span className="dist-bar-wrap">
                          <span
                ...
                {aboveAverage && (
                  <div className="insight-card">
                    ...
                    <div className="insight-highlight-box blue">
                      <span className="insight-big-number">{aboveAverage.aboveCount}<span className="insight-out-of">/{aboveAverage.total}</span></span>
                      <span className="insight-sub-text">applications above avg value of <strong>{aboveAverage.avg}</strong></span>
                    </div>
                  </div>
                )}
